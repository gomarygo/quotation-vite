    ...
    <div style={{ marginTop: 32 }}>
      <div ref={printRef} style={{ background: '#fff', padding: 32, width: 800, margin: '0 auto', fontFamily: 'sans-serif', color: '#222', position: 'relative', borderRadius: 8, boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
        ...
        <div style={{ fontSize: 15, lineHeight: 1.7, marginBottom: 16 }}>
          <div><b>작성일자:</b> {data.serviceStart.replace(/-/g, '년 ').replace(/-(\d{2})$/, '월 $1일')}</div>
          ...
          <div>
            ...
            <img src="/stamp.png" alt="직인" style={{ display: 'inline-block', verticalAlign: 'middle', marginLeft: -30, width: 90, height: 90, maxWidth: 90, maxHeight: 90, objectFit: 'contain', opacity: 0.8, background: 'transparent', border: 'none', zIndex: 10, position: 'relative', boxShadow: 'none', overflow: 'visible' }} />
          </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: 16, fontSize: 16, background: '#fff', tableLayout: 'fixed' }}>
          ...
          <tbody>
            ...
            <tr><td style={{ border: '1px solid #bbb', padding: 8, textAlign: 'center' }}>항목</td><td style={{ border: '1px solid #bbb', padding: 8, textAlign: 'center' }}>{data.itemName}</td></tr>
            <tr><td style={{ border: '1px solid #bbb', padding: 8, textAlign: 'center' }}>플랜 유형</td><td style={{ border: '1px solid #bbb', padding: 8, textAlign: 'center' }}>{data.planType}</td></tr>
            ...
            {data.discounts.map((d, i) => (
              <tr key={i}><td style={{ border: '1px solid #bbb', padding: 8, textAlign: 'center' }}>{d.label}</td><td style={{ border: '1px solid #bbb', padding: 8, color: 'red', textAlign: 'center' }}>-{calculateDiscountAmount(d, subtotal).toLocaleString()}원</td></tr>
            ))}
            ...
            <tr><td style={{ border: '1px solid #bbb', padding: 8, textAlign: 'center' }}>부가세</td><td style={{ border: '1px solid #bbb', padding: 8, textAlign: 'center' }}>{vat.toLocaleString()}원</td></tr>
            ...
            {data.discounts.length > 0 && (
              <tr style={{ background: '#f8f8f8' }}><td style={{ border: '1px solid #bbb', padding: 8, fontWeight: 700, textAlign: 'center' }}>최종 견적가</td><td style={{ border: '1px solid #bbb', padding: 8, fontWeight: 700, fontSize: 18, textAlign: 'center', color: '#1a73e8' }}>{finalAmount.toLocaleString()}원</td></tr>
            )}
          </tbody>
        </table>
        {/* 비고 */}
